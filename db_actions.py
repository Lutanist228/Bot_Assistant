...
class Database:
    ...
    async def add_question(self, user_id: int, user_name:str, question: str):
        if self.connection is None:
            await self.create_connection()
        async with self.connection.execute('INSERT INTO questions (user_id, user_name, question) VALUES (?, ?, ?)', (user_id, user_name, 
                                                                                                                     question)) as cursor:
    ...
    async def update_gpt_answer(self, question_id: int, answer: str):
        if self.connection is None:
            await self.create_connection()
        async with self.connection.execute('''UPDATE questions SET gpt_answer = ?
